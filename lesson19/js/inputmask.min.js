          ...
          off: function off(events, handler) {
            ...
            function removeEvent(ev, namespace, handler) {
              if (ev in eventRegistry == !0)
                if ((elem.removeEventListener ? elem.removeEventListener(ev, handler, !1) : elem.detachEvent && elem.detachEvent('on' + ev, handler), 'global' === namespace)) for (var nmsp in eventRegistry[ev]) eventRegistry[ev][nmsp].splice(eventRegistry[ev][nmsp].indexOf(handler), 1);
            ...
            function resolveNamespace(ev, namespace) {
              var evts = [],
                ...
                hndL;
              if (0 < ev.length)
                if (void 0 === handler) for (hndx = 0, hndL = eventRegistry[ev][namespace].length; hndx < hndL; hndx++) evts.push({ ev: ev, namespace: namespace && 0 < namespace.length ? namespace : 'global', handler: eventRegistry[ev][namespace][hndx] });
                ...
                for (var evNdx in eventRegistry)
                  for (var nmsp in eventRegistry[evNdx])
                    if (nmsp === namespace)
                      if (void 0 === handler) for (hndx = 0, hndL = eventRegistry[evNdx][nmsp].length; hndx < hndL; hndx++) evts.push({ ev: evNdx, namespace: nmsp, handler: eventRegistry[evNdx][nmsp][hndx] });
            ...
            if (isValidElement(this[0]))
              for (var eventRegistry = this[0].eventRegistry, elem = this[0], _events = 'string' == typeof events ? events.split(' ') : [events.type], endx = 0; endx < _events.length; endx++) {
                ...
                if (void 0 !== document && 'global' === namespace) {
                  ...
                  if (((events = events.type ? events : DependencyLib.Event(events)), (events.detail = arguments.slice(1)), 'global' === namespace)) for (var nmsp in eventRegistry[ev]) for (i = 0; i < eventRegistry[ev][nmsp].length; i++) eventRegistry[ev][nmsp][i].apply(elem, arguments);
          ...
          function insertTestDefinition(mtoken, element, position) {
            ...
            if (regexMask)
              0 === element.indexOf('[') || (escaped && /\\d|\\s|\\w]/i.test(element)) || '.' === element
                ? mtoken.matches.splice(position++, 0, { fn: new RegExp(element, opts.casing ? 'i' : ''), static: !1, optionality: !1, newBlockMarker: void 0 === prevMatch ? 'master' : prevMatch.def !== element, casing: null, def: element, placeholder: void 0, nativeDef: element })
                  ...
                  (void 0 === tests[0].alternation || !0 === opts.keepStatic
                    ...
                    : $.each(tests, function (ndx, tst) {
                        if ('' !== tst.def)
                          ...
                          else for (var i = 0; i < locator.length; i++) tst.locator[i] && -1 === locator[i].toString().indexOf(tst.locator[i]) && (locator[i] += ',' + tst.locator[i]);
                      })),
                locator
              );
            }
            ...
            EventHandlers = {
              ...
              keypressEvent: function keypressEvent(e, checkval, writeOut, strict, ndx) {
                ...
                if (k) {
                  ...
                  if (
                    (!1 !== valResult && (resetMaskSet(!0), (forwardPosition = void 0 !== valResult.caret ? valResult.caret : seekNext(valResult.pos.begin ? valResult.pos.begin : valResult.pos)), (maskset.p = forwardPosition)),
            ...
            preValidation: function preValidation(buffer, pos, c, isSelection, opts, maskset, caretPos, strict) {
              ...
              if (isNaN(c) && buffer[pos] !== c) {
                ...
                if (tokenMatch.nextMatch && tokenMatch.nextMatch[0] === c && 1 < tokenMatch.targetMatch[0].length) {
                  ...
                  if (new RegExp(validator).test('0' + buffer[pos - 1])) return (buffer[pos] = buffer[pos - 1]), (buffer[pos - 1] = '0'), { fuzzy: !0, buffer: buffer, refreshFromBuffer: { start: pos - 1, end: pos + 1 }, pos: pos + 1 };
                }
              }
              return !0;
            },
              ...
              if (!1 === opts.__financeInput)
                if (isSelection) {
                  ...
                  if (!opts.digitsOptional) {
                    if (caretPos.begin > radixPos && caretPos.end <= radixPos) return c === opts.radixPoint ? { insert: { pos: radixPos + 1, c: '0', fromIsValid: !0 }, rewritePosition: radixPos } : { rewritePosition: radixPos + 1 };
